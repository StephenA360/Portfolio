...
export function ProjectCard({name, description, tools, media, github, demo}: Project){
    return(
        <div className="flex flex-col border-2 rounded-2xl h-80 w-100 justify-center items-center text-center p-5 leading-relaxed hover:drop-shadow-2xl hover:brightness-110 ease-in-out duration-300 bg-white">
            ...
            <div className="flex flex-wrap justify-center gap-3 bg-white">
                {
                    tools.map((tool, i) => (
                        <div className="flex border h-10 items-center justify-center p-3 hover:drop-shadow-md" key={i}>
                            <p>{tool}</p>
                        </div>
                    ))
                }
